...
def train(model_name, fold_count, train_full_set=False, load_weights_path=None, ndsb3_holdout=0, manual_labels=True):
    ...
    checkpoint = ModelCheckpoint("models" + model_name + "_" + holdout_txt + "_e" + "{epoch:02d}-{val_loss:.4f}.hd5", monitor='val_loss', verbose=1, save_best_only=not train_full_set, save_weights_only=False, mode='auto', period=0.1)
    checkpoint_fixed_name = ModelCheckpoint("models" + model_name + "_" + holdout_txt + "_best.hd5", monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=False, mode='auto', period=0.1)
    model.load_weights("models" + model_name + "_" + holdout_txt + "_best.hd5")
